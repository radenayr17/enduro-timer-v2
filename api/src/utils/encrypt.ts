...
export const signUserToken = (data: TUserTokenInput): TUserTokenOutput => {
  const idToken = jwt.sign(data, JWT_SECRET, { expiresIn: ID_TOKEN_EXPIRES_IN });
  const refreshToken = jwt.sign(data, JWT_SECRET_REFRESH, { expiresIn: REFRESH_TOKEN_EXPIRES_IN });
...
export const verifyUserToken = (token: string, type: "TOKEN" | "REFRESH_TOKEN"): TUserTokenInput => {
  return jwt.verify(token, type === "TOKEN" ? JWT_SECRET : JWT_SECRET_REFRESH) as TUserTokenInput;
};
